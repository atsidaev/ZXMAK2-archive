		...
		public static void Start()
		{
			lock (_syncStart)
			{
                ...
                try
                {
                    ...
                    //string folderName = "C:\\Logs";//Path.Combine(Path.GetDirectoryName(fileName), "Logs");
                    string folderName = Path.Combine(Path.GetDirectoryName(fileName), "Logs");
                    //if (!Directory.Exists(folderName))
                    //    Directory.CreateDirectory(folderName);
                    string salt = Path.GetFileNameWithoutExtension(fileName);
                    ...
                    salt = string.Format(
                        ...
                        dateTime.Minute,
                        dateTime.Second);
                    fileName = Path.Combine(folderName, string.Format("{0}.log", salt));
                    for(int i=0; File.Exists(fileName) && i < 100; i++)
