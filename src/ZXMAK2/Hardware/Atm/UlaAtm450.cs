	...
	public class UlaAtm450 : UlaDeviceBase
	{
		...
		public override float VideoHeightScale
		{
			get
			{
				switch (m_mode)
				{
					case AtmVideoMode.Ega320x200:
						return 1F;
					case AtmVideoMode.Hwm640x200:
					case AtmVideoMode.Txt080x025:
						return 2F;
					case AtmVideoMode.Std256x192:
					default:
			...
			UlaStateBase ulaState)
		{
			switch (m_mode)
			{
				case AtmVideoMode.Ega320x200:	// 0=320x200
				case AtmVideoMode.Hwm640x200:	// 2=640x200
				case AtmVideoMode.Txt080x025:		// 6=640x200 TEXT
					return;
				case AtmVideoMode.Std256x192:	// 3=256x192 (standard)
				default:
					break;
			}
		...
		protected override void OnTimingChanged()
		{
			...
			for (int y = 0; y < 200; y++)
				...
				{
					int pageOffsetBw = (x & 1) == 0 ? 0x01C0 : 0x21C0;
					int pageOffsetAt = ((x + 1) & 1) == 0 ? 0x01C0 : 0x21C0;
					pageOffsetBw += x >> 1;
					pageOffsetAt += (x + 1) >> 1;
					pageOffsetBw += (y >> 3) * 64;
					pageOffsetAt += (y >> 3) * 64;
