...
namespace ZXMAK2.Hardware.Adlers.Views
{
	public partial class FormCpu : FormView, IDebuggerAdlersView
	{
		...
		//stack address context menu
		void listState_MouseClick(object sender, MouseEventArgs e)
		{
			if (m_showStack)
			{
				//Stack shown
				...
				{
					//parse numbers
					string[] numbers = listState.Text.Split( new[] {' '}, StringSplitOptions.RemoveEmptyEntries);
					
					//Follow in disassembler(if an instrucion contain number)
					var menuItemNewAdress = new MenuItem();

					menuItemNewAdress.Index = 0;
					menuItemNewAdress.Text = String.Format("#{0:X4}", numbers[1]);
					...
					contextMenuStack.MenuItems.Add(menuItemFollowInDisassembler);
					contextMenuStack.Show(listState, e.Location);
				}
			}
		}
