...
using DiskSerializer;
using ZXMAK2.Engine;
using System.Xml;
using ZXMAK2;

namespace Sprinter.BDI
{
    #region enums
    ...
    public class SprinterBDI : BusDeviceBase, IConfigurable, IBetaDiskDevice
    {
        public int Z80FQ = 3500000; // 21000000; //todo: #define as (conf.frame*conf.intfq)

        private byte m_bdimode;
        private bool m_openports;
        ...
        public override string Description
        {
            get
            {
                return "Sprinter Beta Disk Interface new";
            }
        }
        ...
        {
            get { return m_ledDiskIO; }
            set { m_ledDiskIO = value; }
        }
        ...
        protected virtual void BusSubscribeWD93IO(IBusManager bmgr)
        {
            ...
            bmgr.SubscribeWRIO(0xFF, 0xBD, new BusWriteIoProc(this.WriteBDIMode));
            bmgr.SubscribeRESET(new BusSignalProc(SprinterBDIReset));

            if (this.FDD[0].SerializeManager.GetSerializer("img") == null)
            {
                ImgSerializer ims = new ImgSerializer(this.FDD[0]);
                this.FDD[0].SerializeManager.AddSerializer(ims);
//                bmgr.AddSerializer(ims);
            }
            if (this.FDD[1].SerializeManager.GetSerializer("img") == null)
            {
                ImgSerializer ims = new ImgSerializer(this.FDD[1]);
                this.FDD[1].SerializeManager.AddSerializer(ims);
/*                try
                {
                    bmgr.AddSerializer(ims);
                }
        ...
        protected virtual void BusReadPortSys(ushort addr, ref byte value, ref bool iorqge)
        {
            ...
            {
                iorqge = false;
                value = this.GetReg(WD93REG.SYS);
            }
        }


        protected void WriteBDIMode(ushort addr, byte value, ref bool iorqge)
        {
            if (iorqge && (this.DOSEN || this.OpenPorts))
            {
                //0x21 - Set 1440 
        ...
        protected virtual void BusWritePortSys(ushort addr, byte value, ref bool iorqge)
        {
            ...
            {
                iorqge = false;
                this.SetReg(WD93REG.SYS, value);
            }
        }


        protected void SprinterBDIReset()
        {
        ...
        private bool m_ledDiskIO = false;
